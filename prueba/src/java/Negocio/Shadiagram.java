...
public class Shadiagram {
    ...
    //metodo buscar por correo 
     public Usuario buscarUsuario(String n) {
        ...
        lu = uj.findUsuarioEntities();
        for (Usuario u : lu) {
            System.out.println("buscar por Correo{ user: " + u.getUsuario() +" n: " + u.getEmail()+" n: " +u.getNombre()+ " n: " + n);

            if (u.getUsuario().equalsIgnoreCase(n)||u.getNombre().equalsIgnoreCase(n)||u.getEmail().equalsIgnoreCase(n)) {
                return u;
            }
        }
        return null;
